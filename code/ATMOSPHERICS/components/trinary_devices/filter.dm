...
*/
	...
	process()
		...
		if(transfer_moles > 0)
			...
			for (var/G in list(PLASMA, OXYGEN, NITROGEN, CARBONDIOXIDE, NITROUS)) //Ensure the gas's presence in the list to prevent errors.
			...
			switch(filter_type)
				...
				if(1) //removing O2
					filtered_out.gasses[OXYGEN] = removed.gasses[OXYGEN]
					removed.gasses[OXYGEN] = 0

				if(2) //removing N2
					filtered_out.gasses[NITROGEN] = removed.gasses[NITROGEN]
					removed.gasses[NITROGEN] = 0
