...
*/
/atom/proc/Beam(atom/BeamTarget,icon_state="b_beam",icon='icons/effects/beam.dmi',time=50, maxdistance=10, var/delete = 1)
	...
	//continue to draw.
		...
		for(N,N<length,N+=32)
			var/obj/effect/overlay/beam/X=new(get_turf(src))
			X.BeamSource=src
			if(N+32>length)
