...
/obj/machinery/computer/slot_machine/attackby(obj/item/I, mob/living/user, params)
	...
	if(istype(I, /obj/item/weapon/spacecash))
		...
		if(prob(2))
			...
			user << "<span class='warning'>[src] spits your credits back out!</span>"

		else
			if(!user.drop_item())
				return
			user << "<span class='notice'>You insert [C.credits] credits into [src]'s slot!</span>"
			balance += C.credits
			updateDialog()
			qdel(C)
...
/obj/machinery/computer/slot_machine/emp_act(severity)
	...
	money -= max(0, give_cash(min(rand(-50, 100 * severity_ascending)), money)) //This starts at -50 because it shouldn't always dispense coins yo
	spin()
...
/obj/machinery/computer/slot_machine/proc/give_prizes(usrname, mob/user)
	...
	if(reels[1][2] + reels[2][2] + reels[3][2] + reels[4][2] + reels[5][2] == "[SEVEN][SEVEN][SEVEN][SEVEN][SEVEN]")
		...
		jackpots += 1
		balance += money - give_cash(JACKPOT)
		money = 0
...
/obj/machinery/computer/slot_machine/proc/give_money(amount)
	...
	var/surplus = amount_to_give - give_cash(amount_to_give)
	money = max(0, money - amount)
	balance += surplus

/obj/machinery/computer/slot_machine/proc/give_cash(amount)
...
/obj/machinery/computer/slot_machine/proc/dispense(amount = 0, mob/living/target, throwit = 0)
	...
	while(amount > 0)
		if(amount >= 1000)
			...
			C = new /obj/item/weapon/spacecash/c500(loc)
		else if(amount >= 200)
			C = new /obj/item/weapon/spacecash/c200(loc)
		else if(amount >= 100)
			C = new /obj/item/weapon/spacecash/c100(loc)
		else if(amount >= 50)
			C = new /obj/item/weapon/spacecash/c50(loc)
		else if(amount >= 20)
			C = new /obj/item/weapon/spacecash/c20(loc)
		else if(amount >= 10)
