...
/obj/item/taperoll/afterattack(var/atom/A, var/mob/user)
	if (istype(A, /obj/machinery/door/airlock))
		if(!user.Adjacent(A))
			user << "<span class='notice'>You're too far away from [A]!</span>"
			return
...
/obj/item/tape/attack_hand(mob/user as mob)
	if (user.a_intent == "help" && src.allowed(user))
		...
		user << "You will need something sharp to cut [src]!"
...
/obj/item/tape/proc/breaktape(var/obj/item/weapon/W, var/mob/user)
	if(user.a_intent == "help" && ((!is_sharp(W) && src.allowed(user))))
		user << "You can't break [src] with that!"
		return
	user.show_viewers("<span class='warning'>[user] breaks [src]!</span>")
