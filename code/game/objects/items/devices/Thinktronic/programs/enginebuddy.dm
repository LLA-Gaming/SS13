/obj/item/device/thinktronic_parts/program/eng/enginebuddy
	name = "Engine Buddy"
	...
	use_app() //Put all the HTML here
			...
			if(gen.anchored)
				...
				for(var/obj/machinery/field/generator/F in orange(7, gen))
					if(!F.connected_gens.len)
						...
						break
		for(var/obj/machinery/singularity/loose in orange(15, PDA.loc))
		...
		for(var/obj/machinery/power/smes/SMES in orange(14, PDA.loc))
			dat += "SMES -"
			if(!SMES.inputting)
		...
		for(var/obj/machinery/particle_accelerator/PA in orange(7, PDA.loc))
		...
		for(var/obj/structure/particle_accelerator/PA in orange(7, PDA.loc))
			if(!PA) return
			switch(PA.construction_state)
				if(0)
					dat += "[PA.name]: <font color='red'><b>not attached to the flooring</b></font><br>"
		...
		dat += "<h4>Emitters</h4>"
		for(var/obj/machinery/power/emitter/emit in orange(19, PDA.loc))
