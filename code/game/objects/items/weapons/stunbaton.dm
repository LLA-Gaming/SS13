...
/obj/item/weapon/melee/baton/examine()
	...
	..()
	if(bcell)
		usr << output("<span class='notice'>The baton is [round(bcell.percent())]% charged.</span>", "ic")
		usr <<"<span class='notice'>The baton is [round(bcell.percent())]% charged.</span>"
	if(!bcell)
		usr << output("<span class='warning'>The baton does not have a power source installed.</span>", "ic")
