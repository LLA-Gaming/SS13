	...
	attack_hand(var/mob/user as mob)
		...
		if(screen == 0)
			...
			for(var/obj/item/part/ammo/implant/tracking/T in world)
				...
				dat += "<A href='?src=\ref[src];ko=\ref[T]'>(<font class='bad'><i>Knock Out</i></font>)</A> |<BR>"
				dat += "********************************<BR>"
		...
		if((usr.contents.Find(src) || (in_range(src, usr) && istype(src.loc, /turf))) || (istype(usr, /mob/living/silicon)))
			...
			if(href_list["inject1"])
				...
				if((I)&&(I.imp_in))
					...
					R << "You stop and hear a voice within your head: '[warning]'"
					R.Stun(3)
					log_say("[usr]/[usr.ckey] sent an implant message to [R]/[R.ckey]: '[warning]'")

			else if(href_list["ko"])
				if((I)&&(I.imp_in))
					var/mob/living/carbon/R = I.imp_in
					R << "You grab at your head and scream, sheer pain filling you. You then fall over."
					R.emote("scream")
					R.Paralyse(25)
					usr << "\red [R] is now unconsious at [R.loc] for 25 Seconds."
					log_say("[usr]/[usr.ckey] knocked out [R]/[R.ckey].")
