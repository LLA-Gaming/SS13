...
/obj/machinery/computer3/communications/captain
	...
	spawn_files			= list(/datum/file/program/card_comp, /datum/file/program/security, /datum/file/program/crew, /datum/file/program/arcade,
							/datum/file/camnet_key, /datum/file/camnet_key/entertainment)
...
/datum/file/program/communications
	name = "COMMaster V3"
	desc = "Used to connect to CentCom and scream for an evacuation shuttle."
	active_state = "comm"
	...
	Topic(var/href, var/list/href_list)
		if(!interactable() || !computer.radio || ..(href,href_list) )
			return
		if (computer.z > 1)
			usr << "\red <b>Unable to establish a connection</b>: \black You're too far away from the outpost!"
			return
		...
		if (authenticated)
			...
			if (authenticated==2)
				...
				if(computer.emagged == 0)
					dat += "<BR>\[ <A HREF='?src=\ref[src];MessageCentcomm'>Send an emergency message to CentCom</A> \]"
				else
