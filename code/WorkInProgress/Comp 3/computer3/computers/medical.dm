...
/*obj/machinery/computer/med_data
	spawn_files	= /datum/file/program/med_data
...
/datum/file/program/med_data/interact()
	...
	if (src.temp)
		...
		dat = text("Confirm Identity: <A href='?src=\ref[];cardslot=1'>[]</A><HR>", src, (computer.cardslot ? text("[]", computer.cardslot.reader.name) : "----------"))
...
/datum/file/program/med_data/Topic(href, list/href_list)
	...
	if (!( data_core.medical.Find(src.active2) ))
		src.active2 = null
	if ((usr.contents.Find(src) || (in_range(src, usr) && istype(computer.loc, /turf))) || (istype(usr, /mob/living/silicon)))
		usr.set_machine(src)
		if (href_list["temp"])
			src.temp = null
		if (href_list["cardslot"])
			if (computer.cardslot)
				computer.cardslot.loc = computer.loc
				computer.cardslot = null
			else
			...
			if (istype(usr, /mob/living/silicon))
				...
				src.rank = "AI"
				src.screen = 1
			else if (istype(computer.cardslot, /obj/item/weapon/card/id))
				src.active1 = null
				src.active2 = null
				if (computer.check_access(computer.cardslot))
					src.authenticated = computer.cardslot.reader.registered_name
					src.rank = computer.cardslot.reader.assignment
					src.screen = 1
		if (src.authenticated)
