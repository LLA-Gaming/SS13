...
/datum/file/program/aifixer
	name			= "HI Sys-Recovery"
	desc			= "Repairs and revives human intelligence cores."
	...
	proc/aifixer_menu()
		...
		if (computer.cradle.occupant)
			var/laws
			dat += "<h3>Stored HI: [computer.cradle.occupant.name]</h3>"
			...
			if (computer.cradle.occupant.stat == 2)
				dat += "<span class='bad'>HI non-functional</span>"
			else
				dat += "<span class='good'>HI functional</span>"
	...
	Topic(var/href, var/list/href_list)
		...
		if ("fix" in href_list)
			...
			while (occupant.health < 100)
				...
				i++
				if(i == 5)
					computer.use_power(150) // repairing an AI is nontrivial.  laptop battery may not be enough.
