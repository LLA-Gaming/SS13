...
/obj/machinery/power/rad_collector/process()
	if(P)
		if(P.air_contents.gasses[PLASMA] <= 0)
			investigate_log("<font color='red'>out of fuel</font>.","singulo")
			P.air_contents.gasses[PLASMA] = 0
			eject()
		else
			P.air_contents.gasses[PLASMA] -= 0.001*drainratio
	return
...
/obj/machinery/power/rad_collector/attack_hand(mob/user as mob)
	if(anchored)
		if(!src.locked)
			...
			investigate_log("turned [active?"<font color='green'>on</font>":"<font color='red'>off</font>"] by [user.key]. [P?"Fuel: [round(P.air_contents.gasses[PLASMA]/0.29)]%":"<font color='red'>It is empty</font>"].","singulo")
			return
		else
	...
	if(istype(W, /obj/item/device/multitool))
		...
		if(!anchored && !isinspace())
			...
			user.visible_message("[user.name] unsecures the [src.name].", \
				...
				"You hear a ratchet")
			disconnect_from_network()
	else if(istype(W, /obj/item/weapon/card/id)||istype(W, /obj/item/device/tablet))
		if (src.allowed(user))
			if(active)
...
/obj/machinery/power/rad_collector/proc/receive_pulse(var/pulse_strength)
	if(P && active)
		...
		power_produced = P.air_contents.gasses[PLASMA]*pulse_strength*20
