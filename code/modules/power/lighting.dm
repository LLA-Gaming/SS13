...
/obj/machinery/light_construct/examine()
	...
	switch(src.stage)
		if(1)
			usr.send_text_to_tab("It's an empty frame.", "ic")
			usr << "It's an empty frame."
			return
		if(2)
			usr.send_text_to_tab("It's wired.", "ic")
			usr << "It's wired."
			return
		if(3)
			usr.send_text_to_tab("The casing is closed.", "ic")
...
/obj/machinery/light/examine()
	...
	if(usr && !usr.stat)
		switch(status)
			if(LIGHT_OK)
				usr.send_text_to_tab("It is turned [on? "on" : "off"].", "ic")
				usr << "It is turned [on? "on" : "off"]."
			if(LIGHT_EMPTY)
				usr.send_text_to_tab("The [fitting] has been removed.", "ic")
				usr << "The [fitting] has been removed."
			if(LIGHT_BURNED)
				usr.send_text_to_tab("The [fitting] is burnt out.", "ic")
