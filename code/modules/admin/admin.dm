...
/datum/admins/proc/show_player_panel(var/mob/M in mob_list)
	...
	body += "<A href='?_src_=holder;oocban=\ref[M]'>OOC Ban</A> | "
...
/datum/admins/proc/Secrets()
	...
	if(check_rights(R_FUN,0))//TODO: Make all this hardcoded random event panel stuff its own proc so we can actually remove poor events without making the code stop compiling
		dat += {"
			...
			<A href='?src=\ref[src];secretsfun=angel'>Trigger a Honking Angel arrival</A><BR>
			...
			<A href='?src=\ref[src];secretsfun=hivebots'>Trigger a Hivebot Invasion</A><BR>
			<BR>
			<B>Fun Secrets</B><BR>
			<BR>
			...
			<A href='?src=\ref[src];secretsfun=striketeam'>Send in DEATH SQUAD</A><BR>
