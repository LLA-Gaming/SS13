...
/datum/round_event/time_travellers/announce()
	priority_announce("An electrical storm has been detected in your area, please repair potential electronic overloads.", "Electrical Storm Alert")

/datum/round_event/time_travellers/tick_queue()
	var/list/candidates = get_candidates_event(BE_TIMEAGENT)
	if(candidates.len >= spawncount)
		unqueue()

/datum/round_event/time_travellers/setup()
	arrival_point_A = findEventArea()
	arrival_point_B = findEventArea()
...
/datum/round_event/time_travellers/start()
	//Make sure the parents are still alive before continuing
	if(mother && mother.current && !ishuman(mother.current) && (mother.current.stat == DEAD))
		kill()
		end()
		return
	if(father && father.current && !ishuman(mother.current) && (mother.current.stat == DEAD))
		kill()
		end()
		return

	//If they arent dead we can move on and start this
	var/list/candidates = get_candidates_event(BE_TIMEAGENT)

	if(candidates.len < spawncount && events.queue_ghost_events && !loopsafety)
		queue()
		return
