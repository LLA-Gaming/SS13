//If you expect me to touch this one, you are dead wrong ~Flavo
			...
			switch(i)
				...
				if(3)
					if(U.stat==2||U.health<=0)
						U << "\red <B>FCAL ?Rr?R</B>: 344--93#?&&21 BR??N |/|/aV? PATT$RN <B>RED</B>\nA-A-aB?rT?NG..."
						unlock_suit()
						break
						...
						if(AI==A)
							switch(i)
								...
								if(2)
									...
									U << "\red <b>WARNING</b>: P????R?Gr?5S 677^%"
									cancel_stealth()
								if(3)
...
/obj/item/clothing/suit/space/space_ninja/examine()
	...
	if(s_initialized)
		...
		if(s_control)
			...
			U.send_text_to_tab("?rr?R ?a??a?? No-?-? f??N? 3RR?r", "ic")
