/mob/living/carbon/alien/beepsky/larva/emote(var/act)
	...
	switch(act) //Alphabetically sorted please.
		...
		if ("moan")
			...
			m_type = 2
		if ("nod")
			message = "<B>[src]</B> beeps affirmatively."
			m_type = 1
//		if ("roar")
//			if (!muzzled)
...
//				m_type = 2
		...
		if ("sign")
			if (!src.restrained())
				message = text("<B>The beepsky larva</B> signs[].", (text2num(param) ? text(" the number []", text2num(param)) : null))
				m_type = 1
//		if ("sit")
...
//			m_type = 1
		...
		if ("sway")
			...
			m_type = 1
		if ("tail")
			message = "<B>[src]</B> waves its stubby baton."
			m_type = 1
		if ("twitch")
