...
/mob/living/carbon/monkey/update_inv_r_hand(var/update_icons=1)
	...
	if(r_hand)
		...
		overlays_standing[M_R_HAND_LAYER]	= image("icon" = GetHeldIconFile("right", t_state), "icon_state" = t_state, "layer" = -M_R_HAND_LAYER)
		overlays += overlays_standing[M_R_HAND_LAYER]
	else
		overlays -= overlays_standing[M_R_HAND_LAYER]
...
/mob/living/carbon/monkey/update_inv_l_hand(var/update_icons=1)
	...
	if(l_hand)
		...
		overlays_standing[M_L_HAND_LAYER]	= image("icon" = GetHeldIconFile("left", t_state), "icon_state" = t_state, "layer" = -M_L_HAND_LAYER)
		overlays += overlays_standing[M_L_HAND_LAYER]
	else
		overlays -= overlays_standing[M_L_HAND_LAYER]
